...
def post_item(user: UserData):
    print(
        f"Email: {user.email}, Password: {user.password}, FirstName: {user.firstname}, LastName: {user.lastname}"
    )
    result = UserCretor_Class.add_item_user(
...
def handle_login(user: UserDataLogin):
    ...
    result = UserCretor_Class.get_logindata(user.email, user.password)
    print("result:", result)
    return result
